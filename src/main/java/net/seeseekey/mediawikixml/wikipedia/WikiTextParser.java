...
public class WikiTextParser {
    ...
    private Pattern stubPattern = null;
    private Pattern disambiguationPattern = null;
    ...
    private void createPatterns() {
        ...
        redirectPattern = Pattern.compile("#" + Language.getLocalizedRedirectLabel(languageCode) + "\\s*\\[\\[(.*?)\\]\\]", Pattern.CASE_INSENSITIVE);
        stubPattern = Pattern.compile("\\-" + Language.getLocalizedStubLabel(languageCode) + "\\}\\}", Pattern.CASE_INSENSITIVE);
        disambiguationPattern = Pattern.compile("\\{\\{" + Language.getDisambiguationLabel(languageCode) + "\\}\\}", Pattern.CASE_INSENSITIVE);
    }
    ...
    private void parseCategories() {
        ...
        pageCats = new HashSet<>();
        Pattern catPattern = Pattern.compile("\\[\\[" + Language.getLocalizedCategoryLabel(languageCode) + ":(.*?)\\]\\]", Pattern.MULTILINE | Pattern.CASE_INSENSITIVE);
    ...
    private void parseLinks() {
        ...
        Pattern catPattern = Pattern.compile("\\[\\[(.*?)\\]\\]", Pattern.MULTILINE); // TODO unify catPattern
    ...
    public String getTextBody() {
        ...
        text = stripBottomInfo(text, Language.getSeeAlsoLabel(languageCode));
        text = stripBottomInfo(text, Language.getFurtherLabel(languageCode));
        text = stripBottomInfo(text, Language.getReferencesLabel(languageCode));
        text = stripBottomInfo(text, Language.getNotesLabel(languageCode));
    ...
    private String cleanHeadings(String text) {
        ...
        Pattern startHeadingPattern = Pattern.compile("^=*", Pattern.MULTILINE); // TODO make pattern static
    ...
    private String stripCite(String text) {
        ...
        String citeConstString = "{{cite";

        int startPos = text.indexOf(citeConstString);
        if (startPos < 0) return text;

        int bracketCount = 2;
        int endPos = startPos + citeConstString.length();
